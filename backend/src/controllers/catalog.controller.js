...
export const catalogController = {
    ...
    async create(req, res) {
        try {
            ...
            const result = await pool.query(`INSERT INTO service_catalogs (name, description, category, price, image_url, created_at)
    ...
    async update(req, res) {
        try {
            const { id } = req.params;
            const { name, description, category, price, image_url } = req.body;
            const result = await pool.query(`UPDATE service_catalogs 
    ...
    async delete(req, res) {
        try {
            const { id } = req.params;
            const result = await pool.query("DELETE FROM service_catalogs WHERE id = $1 RETURNING *", [id]);
            if (result.rows.length === 0) {
                return res.status(404).json({ error: "Catalog not found" });
            }
