...
/**
 * @deprecated Use loginAttemptsService instead
 * This function is maintained for backward compatibility only
 */
export async function checkLoginAttempts(email: string) {
  console.warn('[DEPRECATED] checkLoginAttempts called directly - use loginAttemptsService.checkLoginAttempts with IP address');
  try {
    const result = await pool.query(
    ...
    if (result.rows.length === 0) {
      return { allowed: false, message: 'Admin non trouvé' };
    }
    ...
    return { allowed: false, message: 'Erreur vérification' };
  }
}

/**
 * @deprecated Use loginAttemptsService.recordLoginAttempt instead
 */
export async function incrementLoginAttempts(email: string) {
  console.warn('[DEPRECATED] incrementLoginAttempts called directly - use loginAttemptsService.recordLoginAttempt');
}

/**
 * @deprecated Use loginAttemptsService.resetLoginAttempts instead
 */
export async function resetLoginAttempts(email: string) {
  console.warn('[DEPRECATED] resetLoginAttempts called directly - use loginAttemptsService.resetLoginAttempts');
}
