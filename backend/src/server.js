...
const rawOrigins = process.env.CORS_ORIGINS || 'https://emploiplus-group.com,http://localhost:5173,http://localhost:5174';
...
app.get("/api/jobs", async (req, res) => {
    try {
        // Public listing: only published jobs. Support query params for search and filters.
        ...
        const sql = `SELECT * FROM jobs ${where} ORDER BY created_at DESC LIMIT $${idx} OFFSET $${idx + 1}`;
        const { rows } = await pool.query(sql, params);
        // Ensure boolean fields are explicit for the client
        const normalized = (rows || []).map((r) => ({
            ...r,
            application_via_emploi: !!r.application_via_emploi,
            is_company_owned: !!r.is_company_owned,
...
app.get("/api/formations", async (_, res) => {
    try {
        const { rows } = await pool.query("SELECT * FROM formations WHERE published = true ORDER BY created_at DESC");
        res.json(rows);
    }
    catch (err) {
...
app.get('/api/notifications', userAuth, async (req, res) => {
    try {
        ...
        const { rows: siteRows } = await pool.query(`SELECT id, title, message, target, category, image_url, link, created_at FROM site_notifications WHERE target = $1 OR target = 'all' ORDER BY created_at DESC`, [userRole]);
...
app.get('/api/notifications', userAuth, async (req, res) => {
    try {
        const userId = req.userId;
        const { rows } = await pool.query('SELECT id, title, message, read, created_at FROM notifications WHERE user_id = $1 ORDER BY created_at DESC', [userId]);
        res.json(rows);
    }
    catch (err) {
        console.error('Get notifications error:', err);
...
app.post('/api/publications', userAuth, async (req, res) => {
    try {
        ...
        const { rows } = await pool.query(`INSERT INTO publications (user_id, content, image_url, category, achievement, visibility, hashtags, likes_count, comments_count, is_active, created_at, updated_at) 
       VALUES ($1, $2, $3, $4, $5, $6, $7, 0, 0, true, NOW(), NOW()) 
       RETURNING *`, [userId, content, image_url || null, category, achievement || null, visibility, hashtags || null]);
...
app.put('/api/publications/:id', userAuth, async (req, res) => {
    try {
        ...
        // Check ownership
        const { rows: pubRows } = await pool.query(`SELECT user_id FROM publications WHERE id = $1`, [id]);
        if (!pubRows.length) {
            return res.status(404).json({ success: false, message: 'Publication not found' });
        }
        if (pubRows[0].user_id !== userId) {
...
app.delete('/api/publications/:id', userAuth, async (req, res) => {
    try {
        ...
        const userId = req.userId;
        // Check ownership
        const { rows } = await pool.query(`SELECT user_id FROM publications WHERE id = $1`, [id]);
        if (!rows.length) {
            return res.status(404).json({ success: false, message: 'Publication not found' });
        }
...
app.post('/api/publications/:id/like', userAuth, async (req, res) => {
    try {
        ...
        if (likeRows.length > 0) {
            ...
            await pool.query(`UPDATE publications SET likes_count = GREATEST(likes_count - 1, 0) WHERE id = $1`, [id]);
        }
        else {
            // Like - add a new like
            await pool.query(`INSERT INTO publication_likes (publication_id, user_id) VALUES ($1, $2)`, [id, userId]);
