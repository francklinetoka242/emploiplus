...
export const api = {
  ...
  // admin helpers (no-op for static)
  createFormation: async (data: any) => {
    await simulateNetworkDelay();
    const newF = { ...data, id: 'form-' + Math.random().toString(36).substr(2, 9) };
    MOCK_FORMATIONS.push(newF);
    return { data: newF };
  },

  updateFormation: async (id: string, data: any) => {
    await simulateNetworkDelay();
    const idx = MOCK_FORMATIONS.findIndex((f) => f.id === id);
    if (idx !== -1) {
      MOCK_FORMATIONS[idx] = { ...MOCK_FORMATIONS[idx], ...data };
      return { data: MOCK_FORMATIONS[idx] };
    }
    return { data: null };
  },
  ...
  deleteFormation: async (id: string) => {
    await simulateNetworkDelay();
    const idx = MOCK_FORMATIONS.findIndex((f) => f.id === id);
    if (idx !== -1) MOCK_FORMATIONS.splice(idx, 1);
    return { success: true };
  },
  ...
  publishFormation: async (id: string, published: boolean) => {
    ...
    const idx = MOCK_FORMATIONS.findIndex((f) => f.id === id);
    if (idx !== -1) {
      MOCK_FORMATIONS[idx].published = published;
      return { data: MOCK_FORMATIONS[idx] };
    }
    return { data: null };
  },
