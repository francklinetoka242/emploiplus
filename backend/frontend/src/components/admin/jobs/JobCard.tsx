...
export default function JobCard({ job, onEdit, onTogglePublish, onDelete }: JobCardProps) {
  ...
  const isDeadlinePassed = job.deadline_date ? new Date(job.deadline_date) < new Date() : false;
            ...
            {job.deadline_date && (
              <div className="mt-4 flex items-center gap-2">
                ...
                <span className={isDeadlinePassed ? "text-red-600 font-semibold" : "text-gray-700"}>
                  {isDeadlinePassed ? "Date limite dépassée" : `Date limite: ${formatDate(job.deadline_date)}`}
                </span>
              </div>
            )}
          </div>
