...
export default function VisualCreation({ isLoggedIn = false }: VisualCreationProps) {
  ...
  // NOTE: previously clicks on some cards would redirect unauthenticated
  // users to the login page.  The requirement now is that *all* creation
  // actions are available without logging in, so we no longer guard them.
  // The `isLoggedIn` flag is retained only for styling/label variations.
