/**
 * Section d'upload de documents pour les profils candidats
...
export default function CandidateDocumentsUpload({
  ...
  const handleFileSelect = async (file: File, docKey: string) => {
    const config = CANDIDATE_DOCUMENTS[docKey];
    if (!config) return;
    ...
    const result = await uploadCandidateDocAndSave(
      file,
      docKey,
      userId,
      config.dbColumn
  ...
  return (
    <Card className="p-6">
      <div className="mb-6">
        <h2 className="text-xl font-bold mb-2">Vos Documents</h2>
        <p className="text-sm text-muted-foreground">
          Téléchargez vos documents professionnels en format PDF (max 5 MB). 
          Une coche verte indique que le document est présent.
        </p>
      </div>

      <div className="space-y-4">
        {CANDIDATE_GROUP_ORDER.map(groupName => {
          const documents = CANDIDATE_DOCS_GROUPED[groupName] || [];
      ...
      <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        <p className="text-sm text-blue-900">
          <strong>Conseil :</strong> Téléchargez autant de documents que possible pour augmenter 
          votre crédibilité auprès des employeurs et améliorer votre probabilité d'embauche.
        </p>
      </div>
    </Card>
  );
